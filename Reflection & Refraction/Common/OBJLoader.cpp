...
OBJLoader::Object OBJLoader::GetObject(int index)
{
...
void OBJLoader::LoadCube(const char* vert, const char* frag)
{
	programID = LoadShaders(vert/*"../Common/shaders/phongShading.vert"*/,
		frag	/*"../Common/shaders/phongShading.frag"*/);
	objManager = new(OBJReader);
	Cube.program = programID;
	objManager->ReadOBJFile("../Common/models/cube.obj", &Cube.objMesh, OBJReader::ReadMethod::LINE_BY_LINE, false);
	Cube.objMesh = *(objManager->getMesh());
...
OBJLoader::TypeCube OBJLoader::getCube()
{
	return Cube;
}