...
layout (location = 0) in vec3 aPos; //uvpos 
...
vec3 CalcDirLight(multipleLight light,vec3 aFragPos, vec3 Normal, vec2 realTex);
vec3 CalPointLight(multipleLight light,vec3 aFragPos, vec3 Normal, vec2 realTex);
vec3 CalSpotLight(multipleLight light,vec3 aFragPos, vec3 Normal, vec2 realTex);
 vec2 realTex;
void main()
{ 
	vec3 aFragPos = vec3(model * vec4(aPos, 1.0));
    ...
    for(int i = 0; i < activeLight; i++)
    {
        ...
        {
            combine += CalcDirLight(LightSources[i],aFragPos, Normal, realTex);    
        }
        else if(LightSources[i].lightType == 1) //point
        {
            combine += CalPointLight(LightSources[i],aFragPos, Normal, realTex);
        }
        else if(LightSources[i].lightType == 2) //spotlight
        {
            combine += CalSpotLight(LightSources[i],aFragPos, Normal, realTex);      
        }
    }
	if(IsPlane)
    {
